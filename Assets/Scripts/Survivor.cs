...
//				Debug.Log("player " + input.idx  + " pressed Y!");
		...
		//Nearby torches
		var torches = GetNearActivatableTorches();
	...
	private void Activate(){
		var colliders = GetNearActivatableTorches();
		foreach (var item in colliders) {
			var torch = item.GetComponent<TorchLight>();
			if (torch) if (!torch.turnedOn) {
				torch.SetTurnedOn(true);
			}
		}
	}

	private List<TorchLight> GetNearActivatableTorches(){
		var torchColliders = Physics.OverlapSphere(transform.position + Vector3.up * 1.5f, torchActivateRadius, LayerMask.GetMask("Torches"));
...
//			nextDashAllowedTime = Time.time + dashCooldown;
			...
			SoundManager.I.PlaySound("dash", transform);
//			panel.UseSkill(0, dashCooldown);
	...
	void OnTriggerEnter(Collider other){
		...
		if (other.tag == "Key" && carryItem == ItemType.None){
			...
			Game.I.PickedUpKey(other.GetComponent<Key>().doorIdx);
			...
			var door = other.GetComponent<ExitDoor>();
