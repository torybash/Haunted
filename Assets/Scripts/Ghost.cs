...
public class Ghost : MonoBehaviour {
	...
	[SerializeField] private float torchLigthIncreaseSpeed = 0.3f;
...
//	[SerializeField] private float phantomCooldown = 12f;
	...
	[SerializeField] private float torchLightMaxSlowdown = 0.5f;
	[SerializeField] private float maxLightSlowdown = 0.85f;
	...
	[SerializeField] Image aButtonImg;
...
//				if (hideAudio != null && hideAudio.isPlaying) hideAudio.Stop();
		...
		//Close to torch
		var torches = GetNearActivatableTorches(torchLightRadius);
		foreach (var item in torches) {
			var torch = item.GetComponent<TorchLight>();
			if (torch.turnedOn && lighted < torchLightMaxSlowdown) {
				lighted += Time.deltaTime * torchLigthIncreaseSpeed;
				lighted = Mathf.Max(lighted, torchLightMaxSlowdown);
			}
		}
...
//		var currClr = faceMat.GetColor("_EmissionColor");
		...
		panel.SetShake(lighted);

		GetComponent<PlayerMovement>().slowdownFrac = Mathf.Min( lighted, maxLightSlowdown);
	}

	private List<TorchLight> GetNearActivatableTorches(float dist){
		var torchColliders = Physics.OverlapSphere(transform.position + Vector3.up * 1.5f, dist, LayerMask.GetMask("Torches"));
		return torchColliders.Select(x => x.GetComponent<TorchLight>()).Where(x => x.turnedOn).ToList();
	}
	...
	private void Activate(){
		var colliders = GetNearActivatableTorches(torchActivateRadius);
